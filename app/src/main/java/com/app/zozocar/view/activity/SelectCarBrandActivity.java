...
public class SelectCarBrandActivity extends AppCompatActivity  implements UpdateInform {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length()>0)
                {
                    if (brands!=null)
                    {
                        ...
                        {
                            if (brands.get(i).toString().toLowerCase().contains(query))
                            {
                                filter_list.add(brands.get(i));
                            }
                        }
                        CarBrandAdapter adapter=new CarBrandAdapter(filter_list,SelectCarBrandActivity.this);
                        rv_brand_list.setLayoutManager(new LinearLayoutManager(SelectCarBrandActivity.this));
                        rv_brand_list.setAdapter(adapter);
                    }
                }
